...
main = do count <- getChar
          f <- getChar
          let friend = digitToInt f
          ---let unfriendList = parseDigitToInt (digitToInt count)
          let unfriendList = [digitToInt(flist!!n) | n<-[0..(digitToInt count)]]
          let ans = get_ans friend unfriendList
          putStr (show ans)

parseDigitToInt 0 = []
parseDigitToInt count = do s <- getChar
                           return ((digitToInt s) : (parseDigitToInt (count - 1)))
